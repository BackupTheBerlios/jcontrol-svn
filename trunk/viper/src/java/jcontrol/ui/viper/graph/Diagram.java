...
/**
 * <p>
 * This class <CODE>Diagram</CODE> provides a graphical diagram for JControl.<BR>
 ...
 * @see jcontrol.ui.viper.graph.Histogram
 * @since Viper 1.0
 * @version $Revision$
 */
public class Diagram extends AbstractMeter {
     ...
     * @param resolution The number of blocks that should be shown in the Diagram.
     */
    public Diagram(int x, int y, int width, int height, int resolution) {
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
    ...
    /**
     * Draw all values of the diagram
     */
    private void drawValue(Graphics g) {
        synchronized (m_history) {
            if (m_min != m_max) {
                ...
                int firstValue = m_history[m_historyCounter];
                if (firstValue < m_min)
                    firstValue = m_min;
                else if (firstValue > m_max) firstValue = m_max;

                int value;
                int x1 = x + xoff + m_displayWidth - 1;
                int y1;
                int y2 = Math.scale(m_displayHeight - 2, m_max - m_min, firstValue - m_min);

                for (int i = 1; i < m_history.length; i++) {
                    value = m_history[(m_historyCounter - i + m_history.length) % m_history.length];
                    if (value < m_min)
                        value = m_min;
                    ...
                    if (x1 > x + xoff) {
                        g.clearRect(x1, y + 1, blockSize, m_displayHeight - 2);
                        g.drawLine(x1, y + m_displayHeight - y1 - 1, x1 + blockSize, y
                                + m_displayHeight - y2 - 1);
                    } else {
                        g.clearRect(x + xoff + 1, y + 1, x1 + blockSize - (x + xoff),
                                m_displayHeight - 2);
                        g.drawLine(x + xoff + 1, y + m_displayHeight - y1 - 1, x1 + blockSize, y
                                + m_displayHeight - y2 - 1);
                        break;
                    }
                    y2 = y1;
